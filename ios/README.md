...
iOS 0.1.0 (37) 已于 2026-09-15 上传成功，优化冷启动开屏至约 1.5 秒，详见 [build 37](../docs/RELEASE-0.1.0-37.md)。按本次发布要求，未等待 Apple 处理状态。

iOS 0.1.0 (36) 已于 2026-09-15 确认进入 TestFlight 内部测试，新增 HTML 页面预览，详见 [build 36](../docs/RELEASE-0.1.0-36.md)。本次优化会话列表、聊天阅读与大字号排版。新增 OMP 接入与手机模型配置、本地 Markdown / 图片 / 视频预览、会话品牌图标。最新交互包括星光工具菜单、思考力度分档滑条、原生文本选区与复制、按住说话（上滑取消）及回复朗读。
...
HTML/HTM 文件支持页面与源码切换，使用非持久化 WKWebView 与独立资源 scheme 加载本地相对 CSS、JS、图片和字体。需要同步更新电脑连接器以支持 `/files/web-preview`。页面可运行交互脚本，但不能访问外部网络、设备 API 或手机文件；依赖在线后端或 CDN 的项目应先生成包含本地资源的静态产物。
