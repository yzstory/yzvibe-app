...
暖橙色延续现有 UI 的品牌色方向，奶油白呼应纸感内容表面。提示词目标色为暖橙 `#D66A43`、奶油白 `#FFF8ED`；生成位图有轻微色彩变化，这两个值用于后续品牌延展，不代表每个像素的精确颜色。UI 的实际颜色 token 仍以 [Theme.swift](../ios/Sources/YzVibeKit/Design/Theme.swift) 为准 —— 界面主色是更沉的焦橙 `#C75015`（白字 4.58:1），图标上的暖橙可以更亮，因为它不承载文字。
