...
- 麦克风固定在模型选项右侧、发送按钮下方。只支持按住说话（180 ms 起录），轻点不会开启持续录音。上滑 64 pt 进入取消区，滑回 44 pt 内继续；松开结束转写或撤销本段，无锁定模式。
- 录音使用聊天区域上的橙色声波浮层，不挤动消息或输入栏；取消区变红并提供触觉反馈，适配减少动态效果。松手仅转写，不自动发送。
